/**
 * /api/ncaab-probe.js  —  ONE-OFF spike (delete after the NHL build ships)
 *
 * De-risks the NCAAB build, same pattern as cfb-probe. Returns:
 *   1. oddsApi : icehockey_nhl game-line coverage + P1 markets + player-prop markets
 *                (empty until November tip-off — the ESPN half works today)
...
async function probeOdds() {
  ...
  try {
    ...
    if (games.length) {
      ...
      const p1 = await j(`https://api.the-odds-api.com/v4/sports/${SPORT}/events/${ev}/odds?apiKey=${ODDS_KEY}&regions=us&markets=h2h_h1,totals_h1,spreads_h1&oddsFormat=american`);
      ...
      // Lines-only sport by decision (college props are thin; NCAAF precedent), but
      // record what books actually post so the decision stays evidence-based.
...
async function probeEspnBox() {
  // 2025-26 season dates guaranteed to hold completed games.
  const dates = ["20260406", "20260322", "20260214"]; // title game, tourney, mid-season
  try {
    ...
    // Basketball has one stat category; capture keys + a sample row anyway so the
    // NBA-alias assumption (PTS/REB/AST labels) is verified, not presumed.
    const categories = {};
    ...
    for (const teamBlk of players) {
      for (const cat of (teamBlk.statistics || [])) {
        ...
        const isGoalie = false; // no split in basketball
    ...
    // Linescores confirm 1st-half grading inputs: two half entries (+OT columns).
...
async function probeTeams() {
  try {
    // groups=50 restricts the pull to Division I.
    const r = await j(`${ESPN}/teams?limit=500&groups=50`);
