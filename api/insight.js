...
async function generate(ctx, stats) {
  ...
  if (ctx.betType === "chat") {
    ...
    system = personaLine +
      ...
      "BULL and BEAR are STRICTLY from the perspective of the SELECTED side (the Selection) winning or covering. BULL = every reason the SELECTION wins, INCLUDING the opponent's weaknesses — a poor OPPOSING starter, low opponent scoring, or opponent injuries all HELP the selection and belong in BULL. BEAR = every reason the SELECTION loses: the opponent's strengths, or the selection's own weaknesses. Attribute each figure by which TEAM it belongs to and what it means for THE SELECTION: a weak opposing pitcher is a BULL point for the selection, never a bear point; the selected team's own strength is never a bear point. Each case cites at least one concrete number from DATA and ties it to why the SELECTION wins or loses. Forbidden filler: 'if they play well', 'momentum', 'anything can happen', 'capable'. Two to three sentences each, no platitudes. If there are genuinely no numbers, keep both cases short and honest rather than padded. " +
