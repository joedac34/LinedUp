            ...
            <div className="body">
              ...
              {commishTab==="grade"&&(
                <>
                  ...
                  {weekPicks.length>0&&(()=>{
                    // Deduplicate picks — keep only the most recent submission per user
                    // Group by user first, then find the most recent batch (by created_at)
                    const userPickMap = weekPicks.reduce((acc,p)=>{
                      if(!acc[p.user_id]) acc[p.user_id] = [];
                      acc[p.user_id].push(p);
                      return acc;
                    },{});
                    // For each user, keep only picks from their most recent lock (within 60 seconds of their latest pick)
                    const deduped = Object.values(userPickMap).flatMap(picks=>{
                      const latest = Math.max(...picks.map(p=>new Date(p.created_at).getTime()));
                      return picks.filter(p=>latest - new Date(p.created_at).getTime() < 60000);
                    });
                    const memberGroups = deduped.reduce((acc,p)=>{
                      const uid = p.user_id;
